...
router.post('/', (req, res) => {
    ...
    if (!cat.catName) {
        return res.status(404).send("Uh oh! A cat name was not provided. Please check that you spelled \'cat\' correctly");
    }
    ...
    Promise.all([catsRaw, localCatsRaw]).then(([cats, localCats]) => {
        ...
        for (elem in cats.data) {
            if (elem === cat.catName) {
                return res.status(404).send(cat.catName + " already exists in the online API");
            }
        }
        ...
        cat.save().then((cat) => {
            ...
            return res.status(404).send("Uh oh! Error occured while trying to get cats");
        });
    });
});
