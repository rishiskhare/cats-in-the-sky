...
router.post('/', (req, res) => {
    ...
    Promise.all([vegesRaw, localVegesRaw, excludedVegesRaw]).then(([veges, localVeges, excludedVeges]) => {
        const vege = new Vege({
            vegeName: req.body.vege
        });
        ...
        if (!vege.vegeName) {
            return res.status(404).send("Uh oh! A vege name was not provided. Please check that you spelled \'vege\' correctly");
        }
        

        // Check if the vege has already been added
        const newVegeInLocal = localVeges.find(v => v.vegeName === vege.vegeName);
        ...
        const newVegeInExcluded = excludedVeges.find(v => v.vegeName === excludedVege.vegeName);
...
router.delete('/', (req, res) => {
    ...
    Promise.all([vegesRaw, localVegesRaw, excludedVegesRaw]).then(([veges, localVeges, excludedVeges]) => {
        ...
        if (!excludedVege.vegeName) {
            return res.send("Uh oh! You didn't include a vege name... Make sure you spelled \'vege\' correctly");
        }
        ...
        if (!onlineVege) {
            ...
            if (!localVege) {
                return res.status(404).send('Uh oh! The vege with the provided name is not found... Please try again');
            }
            ...
            localVege.remove({vegeName: localVege.vegeName}).then((vege) => {
        ...
        // If the vege is found in the online API veges data, add the vege as an entry to the vegesExcluded table
        const newExcludedVege = excludedVeges.find(v => v.vegeName === excludedVege.vegeName);
            
        // If already in the excludedveges collection, send a message
        if (newExcludedVege) {
            return res.send(newExcludedVege.vegeName + ' already exists in the excluded veges');
        }

        excludedVege.save().then((vege) => {
